...
export const VerificationConfirm = (props: VerificationConfirmProps) => {
	...
	const renderButton = () => {
		if (!isSubmitClicked) {
			return (
				<button onClick={handleClickButton}>{intl.formatMessage({ id: 'kyc.screen.VerificationConfirm.btn' })}</button>
			);
		}
		return null;
	};
		...
		if (isSubmitClicked) {
			if (isSendVerifyKycLoading) {
				...
				if (isVerifyKycSubmmited) {
					return (
						<div>
							<div className="verification-confirm__body__heading">
								{intl.formatMessage({ id: 'kyc.screen.VerificationConfirm.heading.1' })}
							</div>
							<div className="verification-confirm__body__desc mt-5">
		...
		return (
			<div>
				...
				<div className="d-flex justify-content-between">
					...
					<h5> {getTextIfMissing(_startCase(document))}</h5>
				</div>
				<div className="d-flex justify-content-between">
					<h5>{`${_startCase(document)} Number`}: </h5>
	...
	return (
		<div className="verification-confirm mb-5">
			<h2 className="mb-5">{intl.formatMessage({ id: 'kyc.screen.VerificationConfirm.Apply' })}</h2>
			<div className="d-flex justify-content-center">{renderImage()}</div>
			<div className="verification-confirm__body">
				{renderConfirmText()}
				{renderButton()}
			</div>
		</div>
