...
const DepositNotes = props => {
	...
	const { min_deposit_amount, deposit_fee } = currencies.find(cur => _toLower(cur.id) === _toLower(currency)) || {
			...
			<div className="td-mobile-screen-deposit__notes__body text-white">
				...
				<p>
					{intl.formatMessage(
						...
						{
							min_deposit_amount: Number(min_deposit_amount) + Number(deposit_fee) ?? 'Unavailble',
							currency: _toUpper(currency),
						},
					)}
				</p>
				<p>
					{intl.formatMessage(
						...
						{ deposit_fee: Number(deposit_fee) ?? 'Unavailble', currency: _toUpper(currency) },
					)}
				</p>
				<p>
