...
class Withdraw extends React.Component<DispatchProps & WithdrawProps & ReduxProps & IntlProps, WithdrawState> {
	...
	private renderWarning() {
		if (!this.isSafeWithdraw())
			return (
				<span style={{ color: 'red', fontSize: '0.75rem', margin: '0 0.5rem' }}>
					{this.translate('page.body.wallets.tabs.withdraw.warning.remains')}
					<a href="/profile/kyc" style={{ color: '#fff', fontSize: '0.75rem', marginLeft: '0.5rem' }}>
						{this.translate('page.body.wallets.tabs.withdraw.forward.kyc')}
					</a>
				</span>
			);
		const { parentWalletBalance } = this.props;

		const balance = _toNumber(parentWalletBalance) || 0;
		if (balance < Number(this.state.amount)) {
			return (
				<span style={{ color: 'red', fontSize: '0.75rem', margin: '0 0.5rem' }}>
					{this.translate('page.body.wallets.tabs.withdraw.warning.balance')}
				</span>
			);
		}
		return null;
	...
	public render() {
		...
		const isWithdrawButtonDisabled =
			...
			_isEmpty(otpCode) ||
			!this.isSafeWithdraw() ||
			!_toNumber(this.props.parentWalletBalance) ||
			_toNumber(this.props.parentWalletBalance) < Number(this.state.amount);
				...
				{this.props.otp && (
					<div className="td-withdraw-column">
						...
						<div className={withdrawAmountClass}>
							<div className="d-flex flex-row justify-content-between mb-2">
								...
								<span className="text-white text-title-input">
									...
									{_toNumber(this.props.parentWalletBalance || 0).toFixed(this.props.fixed)}{' '}
						...
						<div className="my-2 text-desc-input">
							{this.translate('page.body.wallets.tabs.withdraw.note.minWithdraw')} {this.props.minWithdrawAmount}
						</div>
						<div className="my-2 text-desc-input">
							{withdrawFeeLabel} {this.renderFee()}
						</div>
						<div className="my-2 text-desc-input">
							{'Convert to USD'} {this.renderPriceCurrencyByUSD()}
						</div>
