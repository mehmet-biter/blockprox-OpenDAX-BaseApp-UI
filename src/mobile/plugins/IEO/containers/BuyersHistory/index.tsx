...
export const BuyersHistory: React.FC<BuyersHistoryProps> = (props: BuyersHistoryProps) => {
			...
			return (
				<React.Fragment key={index}>
					<tr className="text-center" style={{ color: '#ffff', border: '1px solid #848e9' }}>
						<td>{item.uid}</td>
						<td>
							{`${formatNumber(Number(item.quantity).toString())}`} <br /> {`(${_toUpper(item.base_currency)})	`}
						</td>
						<td>
							{`${Number(item.total)}`} <br /> {`(${_toUpper(item.quote_currency)})`}
						</td>
						<td>
	...
	return (
		<>
			<div id="buyers-ieo-mobile-history">
