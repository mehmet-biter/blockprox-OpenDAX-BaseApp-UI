...
export const DepositInfo: React.FC<DepositInfoProps> = (props: DepositInfoProps) => {
	...
	const allChildCurrencyIds = allChildCurrencies.map(cur => cur.id);
	...
	const textMinDeposit = `${intl.formatMessage(
		...
		{
			min_deposit_amount:
				(Number(currency.min_deposit_amount) + Number(currency.deposit_fee)) ?? 'Unavailble',
	...
	const textDepositFee = `${intl.formatMessage(
		...
		{ deposit_fee: Number(currency.deposit_fee) ?? 'Unavailble', currency: _toUpper(currency_id) },
	)}`;
	...
	const options = currencies
		...
		.map(currency => {
			const newCurrency = {
				...
				label: (
					<span>
						<CurrencyIcon style={{ width: '2rem', borderRadius: '50%' }} currency_id={currency.id} />{' '}
