...
export const WithdrawAddress: React.FC<WithdrawAddressProps> = (props: WithdrawAddressProps) => {
	...
	const { payload, loading } = useSelector(selectWithdrawLimitRemains);
	const { remains, limit } = payload;
	React.useEffect(() => {
		if (!loading) dispatch(withdrawLimitFetchRemains());
	}, [currencyState]);
	// state
	const [withdrawState, setState] = React.useState<WalletsState>({
		...
		tab: intl.formatMessage({ id: 'page.body.wallets.tabs.withdraw' }),
		withdrawDone: false,
		total: '',
		currentTabIndex: 0,
	...
	const renderChildWalletTabPanels = () => {
		return childWallets
			? childWallets.map(childWallet => {
					const currency = currencies.find(cur => _toLower(cur.id) === _toLower(childWallet.id));
					if (currency && currency.withdrawal_enabled === false) return null;
					return (
					...
					<div className="row">
						<div className="col-12">
							<div className="react-tabs">
								...
								{isFetchChildCurrenciesLoading || loading ? (
