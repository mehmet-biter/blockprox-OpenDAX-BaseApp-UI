...
import { FiatDepositScreen } from 'plugins/Fiat/screens/FiatDepositScreen';
import { FiatWithdrawScreen } from 'plugins/Fiat/screens/FiatWithdrawScreen';
...
const PublicRoute: React.FunctionComponent<any> = ({ component: CustomComponent, loading, isLogged, ...rest }) => {
	...
	if (isLogged) {
		return (
			<Route {...rest}>
				<Redirect to={'/wallets/withdraw/fiat/INR'} />
			</Route>
		);
	}
	...
	public render() {
			...
			return (
				<div className={'container-fluid pg-layout pg-layout--mobile'}>
					<Switch>
						...
						<PublicRoute path="/announcements/detail/:id" exact component={AnnouncementMobileDetail} />
		...
		return (
			<div className={`container-fluid pg-layout ${tradingCls}`}>
				<Switch>
					<PublicRoute exact={true} path="/announcements" component={NewAnnouncement} />
					<PublicRoute exact={true} path="/announcements/detail/:id" component={AnnouncementDetail} />
