...
import { currenciesFetch, Market, selectMarkets, selectMarketTickers, setCurrentMarket } from '../../modules';
...
export const MarketsHotOnlist: React.FC<any> = () => {
	...
	const MarketChart = (data: Array<{ pv: string }>, marketID: string) => {
		const market = markets.find(
			market =>
				_toLower(market.quote_unit) === _toLower(marketID.split('/')[1]) &&
				_toLower(market.base_unit) === _toLower(marketID.split('/')[0]),
		);
